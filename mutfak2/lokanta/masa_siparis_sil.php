<?	
	...
	if (mysql_escape_string($_GET[SiparisID])) {
		$SiparisHareketID = $_GET[SiparisID];
		$MasaID = mysql_escape_string($_GET[MasaID]);
		$SqlSorgu = "";
		try {
			$SqlSorgu = mysql_query("DELETE FROM siparishareket WHERE id='$SiparisHareketID'");
                        ...
                        <div class="modal-dialog modal-sm">
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h4 class="modal-title center" id="myModalLabel2">Ürünler Detay <?=$Siparis[id]; ?></h4>
                                </div>
                                ...
                                <div class="modal-footer">
                                    <a  onclick="SiparisDuzenle('<?=$Siparis[id]?>', '<?=$MasaID?>', document.getElementById('miktar<?=$i?>').value)"  class="btn btn-primary"> Kaydet</a>
                                </div>
                            </div>
                        </div>
                    </div>
